...
```{r data}
message <- read_csv(here::here("week5", "pa2", "data", "scrambled_message.txt"))
...
2.  No word should be longer than 16 characters. Drop all extra characters off the end of each word.
    ...
    ```{r}
    message <- message |>
      mutate( Word = str_trunc(Word, width = 16, side = 'right'))
    ```
...
4.  Replace all instances of exactly 2 a's with exactly 2 e's.
    ...
    ```{r}
    message <- message |>
      mutate( Word = str_replace(Word, pattern = "(Aa|aa)", "ee"))
...
5.  Replace all z's with t's.
    ...
    ```{r}
    message <- message |>
      mutate( Word = str_replace_all(Word, pattern = "Z|z", "t"))
...
6.  Every word that ends in b, change that to a y.
    ...
    ```{r}
    message <- message |>
      mutate( Word = str_replace(Word, pattern = "B|b$", "y"))
    ```
...
7.  Every word that starts with k, change that to a v.
    ...
    ```{r}
    message <- message |>
      mutate( Word = str_replace(Word, pattern = "^k|K", "v"))
    ```

8.  Use `str_c()` to recombine all your words into a message.

    ```{r}
    kek <- str_flatten(message$Word, ' ')
